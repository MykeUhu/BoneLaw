...
float UMMC_StoneMaxMorale::CalculateBaseMagnitude_Implementation(const FGameplayEffectSpec& Spec) const
{
	...
	const float R =
		FMath::Clamp(
			0.35f * StoneMMC::Primary01(Willpower) +
			0.25f * StoneMMC::Primary01(Social) +
			0.25f * StoneMMC::KnowledgeDelta01(KnowledgeCourage) +
			0.15f * StoneMMC::KnowledgeDelta01(KnowledgeSocial),
			0.f, 1.f);
